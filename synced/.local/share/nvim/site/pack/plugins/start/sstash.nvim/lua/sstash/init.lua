...
local STATE_DIR = vim.fs.joinpath(vim.fn.stdpath("state"), "/sstash.nvim/")
...
local function find_session_path()
    ...
    while #current > 1 do
        local maybe = vim.fs.joinpath(STATE_DIR, current, session_name)
...
M.commands = {
    ...
    write = function()
        local data_dir = vim.fs.joinpath(STATE_DIR, M.config.get_cwd())
